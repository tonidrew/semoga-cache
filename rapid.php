...
function AgcFeed() {
	...
	if (file_exists('AGODA.txt')) {
$i=1;
$interval = -1; //interval -5 jam ke belakang

		$myfile = fopen("AGODA.txt", "r") or die("Unable to open file!");
		include('PENGATURAN/config.php');		
		while( $i < $feed) {
			$seek = rand(0, filesize("AGODA.txt"));
			if ($seek > 0) {
				fseek($myfile, $seek);
				fgets($myfile);
			}
		...
		print '		<li><a href="'.$domain.'/promo/'.$url.'">'.$domain.'/promo/'.$url.'</a></li>';
