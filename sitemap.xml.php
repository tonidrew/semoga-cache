...
$xml = str_replace ('mapxml','.txt',$urlx);
$xml = ''.$direktori.''.$xml.'';
...
<!-- Generator by Sitemap Generator  -->
';
echo "\n";
$priority = '0.1';
...
echo "\t<url>\n";
echo "\t\t<loc>".$servername."/diagram/".$post."</loc>\n";
echo "\t\t<lastmod>".date("Y-m-d\TH:m:s+00:00", getTanggal($interval*$i))."</lastmod>\n";
echo "\t\t<changefreq>never</changefreq>\n";
echo "\t\t<priority>".$priority."</priority>\n";
echo "\t\t<image:image>\n";
echo "\t\t<image:loc>".$servername."/".seo_friendly_url($link).".jpg</image:loc>\n";
echo "\t\t<image:title><![CDATA[".ucwords($link)."]]></image:title>\n";
echo "\t\t</image:image>\n";
echo "\t</url>\n";
}
echo '</urlset>';
